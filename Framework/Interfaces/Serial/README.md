...
`SerialBitOut` provides the stream methods expected by `Console`. So you can use it for [console](../../Services/Console) transport as well:
```cpp
#include "Framework/core.h"
...
`PIN` must be a GPIO pin descriptor such as `D1`, `B0`, or another type defined in `Framework/AVR/GPIO/pins.h`.
